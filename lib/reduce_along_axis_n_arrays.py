...
def max_array_id(array_tuple):
    dims_list=np.array(map(lambda x: x.ndim,array_tuple))
    size_list=np.array(map(lambda x: x.size,array_tuple))
...
def structure(*args):
    new_struct=np.empty(args[0].shape,dtype=','.join(map(get_dtype_string,args)))
